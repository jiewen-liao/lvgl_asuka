...
declare -a _LVGL_LUNCH_MENU=("host-sdl" "host-wayland" "rk3568-drm" "rk3568_drm_rga" "rk3568_drm_egl")
declare -A _LVGL_LUNCH_DESC=(
    ...
    ["rk3568_drm_egl"]="RK3568 target using DRM/KMS + EGL/GLES"
)
declare -A _LVGL_LUNCH_DEFAULTS=(
    ...
    ["rk3568_drm_egl"]="$LVGL_PORT_ROOT/configs/lunch/rk3568_drm_egl.defaults"
)
...
_lvgl_apply_lunch_target() {
    ...
    case "${target}" in
        ...
        rk3568_drm_egl)
            ...
            export LVGL_LUNCH_BACKENDS="drm,egl,opengles"
            ;;
        *)
...
_lvgl_configure_cmake() {
    ...
    if [[ "${target}" == "rk3568-drm" || "${target}" == "rk3568_drm_rga" || "${target}" == "rk3568_drm_egl" ]]; then
        local default_toolchain="${LVGL_PORT_ROOT}/rk3568_toolchain.cmake"
        if [[ ! -f "${default_toolchain}" ]]; then
